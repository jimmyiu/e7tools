<template>
  <v-sheet class="mx-auto pa-2">
    ...
    <v-row v-for="(item, key) in stats" :key="key" dense>
      ...
      <v-col>
        <v-text-field v-model="value[item[0].value].min" dense hide-details label="min" outlined />
      </v-col>
      <v-col>
        <v-text-field v-model="value[item[0].value].max" dense flat hide-details label="max" outlined />
      </v-col>
      <v-col cols="auto" style="margin: auto">
        <v-img max-height="18" max-width="18" :src="require(`@/assets/img/stat/${item[1].value}.png`)" />
      </v-col>
      <v-col>
        <v-text-field v-model="value[item[1].value].min" dense hide-details label="min" outlined />
      </v-col>
      <v-col>
        <v-text-field v-model="value[item[1].value].max" dense flat hide-details label="max" outlined />
      </v-col>
    </v-row>
    <v-row dense>
